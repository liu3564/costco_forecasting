"""
Predict delay % from baseline for a segment using weather and journey start date/time.
Supports Ridge, Random Forest, and XGBoost (each stored in its own .joblib file).

Features: 7 weather + journey_start_hour (0-23), journey_start_day_of_week (0-6), journey_start_month (1-12).
...
def parse_journey_start(journey_start):
    """
    Parse journey_start to (hour 0-23, day_of_week 0-6 Monday=0, month 1-12, week_of_year 1-52).
    journey_start can be: None, datetime, or dict with date/time or hour/day_of_week/month/week_of_year.
    """
    if journey_start is None:
        return DEFAULT_JOURNEY_HOUR, DEFAULT_JOURNEY_DAY_OF_WEEK, DEFAULT_JOURNEY_MONTH, DEFAULT_WEEK_OF_YEAR
    if isinstance(journey_start, datetime):
        iso = journey_start.isocalendar()
        return journey_start.hour, journey_start.weekday(), journey_start.month, iso[1]
    if isinstance(journey_start, dict):
        ...
        if date_str and time_str and str(time_str).strip():
            try:
                ...
                iso = dt.isocalendar()
                return dt.hour, dt.weekday(), dt.month, iso[1]
            ...
            try:
                ...
                iso = dt.isocalendar()
                return DEFAULT_JOURNEY_HOUR, dt.weekday(), dt.month, iso[1]
            except (ValueError, TypeError):
                pass
    return DEFAULT_JOURNEY_HOUR, DEFAULT_JOURNEY_DAY_OF_WEEK, DEFAULT_JOURNEY_MONTH, DEFAULT_WEEK_OF_YEAR
...
def predict_delay_pct(weather_dict, model_name="ridge", journey_start=None):
    """
    ...
    journey_start: optional datetime or dict with "date" (YYYY-MM-DD) and "time" (HH:MM),
                   or dict with hour, day_of_week, month, week_of_year.
    Returns: predicted delay % (typically 0--50).
    """
    art = _load_artifact(model_name)
    feat_dict = _weather_to_feature_dict(weather_dict, journey_start)
...
def _weather_to_feature_dict(weather_dict, journey_start):
    """Build full feature dict from weather dict + journey_start (adds hour, day_of_week, month, week_of_year)."""
    h, dow, mo, woy = parse_journey_start(journey_start)
...
def predict_all_models(weather_dict, journey_start=None):
    """
